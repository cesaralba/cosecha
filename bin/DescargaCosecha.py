...
def parse_arguments():
    ...
    parser.add('-c','--config', dest='config', action="store", env_var='CS_CONFIG', required=False, help='Fichero de configuración', default="etc/cosecha.cfg")
    ...
    parser.add('-o', dest='destdir', type=str, env_var='CS_DESTDIR', help='Root directory to store things', required=False)
    parser.add('-i', dest='destimgs', type=str, env_var='CS_DESTDIRIMG', help='Location to store images (supersedes ${CS_DESTDIR}/images', required=False)
    parser.add('-m', dest='destmeta', type=str, env_var='CS_DESTDIRMETA', help='Location to store metadata files (supersedes ${CS_DESTDIR}/metadata', required=False)
    parser.add('-s', dest='deststate', type=str, env_var='CS_DESTDIRSTATE', help='Location to store state files (supersedes ${CS_DESTDIR}/state', required=False)
